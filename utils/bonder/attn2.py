...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        # qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)   # [3, B, num_head, C // self.num_heads]
        # q, k, v = x.unsqueeze(1), x.unsqueeze(1), x.unsqueeze(1)

        qkv = torch.cat([x, x, x], dim=-1).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)   # [3, B, num_head, C // self.num_heads]
...
class CrossAttnBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.norm2_q = norm_layer(dim, elementwise_affine=False) 
        self.norm2_kv = norm_layer(dim, elementwise_affine=False)

        self.norm_mlp = norm_layer(dim, elementwise_affine=False)
